...
public class TutorService {
    ...
    public Tutor buscarPorId(String id) {
        return repository.findById(id).orElseThrow(() -> new RuntimeException("Tutor não encontrado"));
    }

    public Tutor cadastrar(Tutor tutor) {
        if (tutor.getNomeCompletoTutor() == null || tutor.getNomeCompletoTutor().isEmpty()) {
            throw new RuntimeException("O nome do tutor é obrigatório!");
        }
        if (tutor.getCpf() == null || tutor.getCpf().isEmpty()) {
        ...
        if (tutor.getTelefone() == null || tutor.getTelefone().isEmpty()) {
            throw new RuntimeException("O telefone é obrigatório!");
        }
        if (tutor.getRua() == null || tutor.getRua().isEmpty()) {
            throw new RuntimeException("A rua é obrigatória!");
        }
        if (tutor.getNumero() == null || tutor.getNumero().isEmpty()) {
            throw new RuntimeException("O número é obrigatório!");
        }
        if (tutor.getBairro() == null || tutor.getBairro().isEmpty()) {
